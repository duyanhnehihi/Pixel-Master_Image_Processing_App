...
class LoadQt(QMainWindow):
    def __init__(self):
        ...
        loadUi('assets/main.ui', self)
        self.setWindowIcon(QtGui.QIcon("assets/python-icon.png"))
        ...
        # Image Restoration 1
        self.actionAdaptive_Wiener_Filtering.triggered.connect(
            self.weiner_filter)
        self.actionMedian_Filtering.triggered.connect(self.median_filtering)
        self.actionAdaptive_Median_Filtering.triggered.connect(
            self.adaptive_median_filtering)
        ...
        self.actionDirectional_Filtering_2.triggered.connect(
            self.directional_filtering)
        self.actionDirectional_Filtering_3.triggered.connect(
            self.directional_filtering2)
        self.actionDirectional_Filtering_4.triggered.connect(
            self.directional_filtering3)
    ...
    def displayImage(self, window=1):
        ...
        # chuyển đổi hiệu quả một ảnh RGB thành một ảnh BGR.
        img = img.rgbSwapped()
        if window == 1:
            self.imgLabel.setPixmap(QPixmap.fromImage(img))
            # căn chỉnh vị trí xuất hiện của hình trên lable
    ...
    def open_img(self):
        fname, filter = QFileDialog.getOpenFileName(
            self, 'Open File', 'C:\\Users\DELL\PycharmProjects\DemoPro', "Image Files (*)")
        if fname:
    ...
    def big_Img(self):
        self.image = cv2.resize(self.image, None, fx=1.5,
                                fy=1.5, interpolation=cv2.INTER_CUBIC)
        self.displayImage(2)

    def small_Img(self):
        self.image = cv2.resize(self.image, None, fx=0.75,
                                fy=0.75, interpolation=cv2.INTER_CUBIC)
        self.displayImage(2)

    def SIZE(self, c):
        self.image = self.tmp
        self.image = cv2.resize(self.image, None, fx=c,
                                fy=c, interpolation=cv2.INTER_CUBIC)
        self.displayImage(2)
    ...
    def AboutMessage2(self):
        QMessageBox.about(self, "About Author", "Người hướng dẫn:   ThS. Phan Hồ Viết Trường \n\n"
                                                "Người thực hiện:\n"
                          "\tNgô Tiến Dũng - 207CT27665\n"
                          "\tQuách Nhật Hào - 207CT40239\n"
                          "\tTrần Duy Anh - 207CT27586\n"
                          "\tNguyễn Thị Kim Hoàng - 207CT55056\n"
                          "\tPuih Thân - 207CT28536\n"

                          )

    def QuestionMessage(self):
        message = QMessageBox.question(
            self, "Exit", "Bạn có chắc muốn thoát", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
################################ Chương 2 ##############################################################################
    def rotation(self):
        ...
        M = cv2.getRotationMatrix2D(
            (cols / 2, rows / 2), 90, 1)  # thay đổi chiều của ảnh
    ...
    def translation(self):
        ...
        img_translation = cv2.warpAffine(
            self.image, translation_matrix, (num_cols, num_rows))
    ...
    def Canny(self):
        ...
        if self.canny.isChecked():
            ...
            self.image = cv2.Canny(
                can, self.canny_min.value(), self.canny_max.value())
        self.displayImage(2)
                    ...
                    if i + z - indexer < 0 or i + z - indexer > len(self.image) - 1:
                        ...
                        if j + z - indexer < 0 or j + indexer > len(self.image[0]) - 1:
                            ...
                            for k in range(filter_size):
                                temp.append(
                                    self.image[i + z - indexer][j + k - indexer])
...
#################################### Image Restoration 2#################################################################
    def inv_filter(self):
        ...
        for i in range(0, 3):
            ...
            h_padded[:self.image.shape[0],
                     :self.image.shape[1]] = np.copy(self.image)
...
######################################## Filter ##########################################################################
    ...
    def butter_filter(self):
        ...
        self.image = 20 * \
            np.log(cv2.magnitude(self.image[:, :, 0], self.image[:, :, 1]))
        self.displayImage(2)
